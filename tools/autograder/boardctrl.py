...
#########################################################################################
# UW Specific Board Control Classes
#########################################################################################
...
class BoardCtrlLocalUW(AbstractLocalBoardController):
    def __init__(self, bfroot, buildroot):
        super().__init__(None, '')
        self.bfroot = pathlib.Path(bfroot)
        self.build = pathlib.Path(buildroot)

    # returns the command for obtaining the console to the board as a list of strings
    # needs to be implemented by the subclass
    def _console_cmd(self) :
        return [str(self.bfroot / "tools/serverboot.sh"), str(self.build / "armv8_imx8x_aos_autograder_None_img")]
